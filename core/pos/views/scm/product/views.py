            ...
            if action == 'search':
                ...
                with transaction.atomic():
                    ...
                    for row in range(2, excel.max_row + 1):
                        ...
                        #codigo de barra columna 2
                        product.cod_barra = excel.cell(row=row, column=2).value
                        #nombre columa 3
                        product.name = excel.cell(row=row, column=3).value
                        name = excel.cell(row=row, column=4).value
                        ...
                        product.price = float(excel.cell(row=row, column=5).value)
                        product.pvp = float(excel.cell(row=row, column=6).value)
                        product.stock = int(excel.cell(row=row, column=7).value)
                        product.save()
            else:
...
class ProductCreateView(PermissionMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'add':
                product = Product()
                product.cod_barra = request.POST['cod_barra']
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            for product in Product.objects.filter().order_by('id'):
                ...
                worksheet.write(row, 1, product.cod_barra, row_format)
